  ...
  module Pages
    class MenuPresenter
      ...
      def render_menu_item_link(menu_item)
        link_to(menu_item.title, context.refinery.url_for(menu_item.url), {:class => link_tag_css, :style => background_colour(menu_item)})
      end
	  ...
	  def background_colour(menu_item)
		  "background: ##{menu_item.page_colour};" if menu_item.page_colour
	  end
